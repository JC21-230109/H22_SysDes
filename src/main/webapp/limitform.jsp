...
function build(){
  ...
  for(let d=1; d<=last; d++){
    ...
    tr.innerHTML = `
      ...
      <td><input type="number" name="am_${iso}" min="0"></td>
      <td><input type="number" name="pm_${iso}" min="0"></td>
    `;
    body.appendChild(tr);
  }
}
...
%>
<option value="<%=i%>" <%= (i == nowYear ? "selected" : "") %>>
  <%=i%>
</option>
<% } %>
</select>
...
for(int i = 1; i <= 12; i++){
%>
<option value="<%=i%>" <%= (i == nowMonth ? "selected" : "") %>>
  <%=i%>
</option>
<% } %>
</select>
...
<!-- ★ 表示用なので submit にしない -->
<button type="button" onclick="build()">決定</button>
